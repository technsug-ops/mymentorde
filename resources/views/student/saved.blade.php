...
@push('head')
<style>
.sv-hero{background:linear-gradient(to right,var(--u-brand-2,#112e56),var(--u-brand,#1f6fd9));border-radius:14px;padding:28px 28px;color:#fff;margin-bottom:24px;}
...
<div style="font-size:.82rem;color:var(--u-muted,#888);margin-bottom:12px;display:flex;align-items:center;gap:6px;">
    ...
    <a href="{{ route('student.discover') }}" style="color:var(--u-brand,#2563eb);text-decoration:none;display:inline-flex;align-items:center;gap:4px;"><x-icon name="compass" size="14" aria-label="Keşfet" /> Keşfet</a>
    <span>›</span>
    @endpageVisible
...
<div style="text-align:center;padding:60px 24px;background:var(--u-card,#fff);border:1px solid var(--u-line,#e5e7eb);border-radius:12px;">
    <div style="display:flex;justify-content:center;margin-bottom:12px;color:var(--u-muted,#9ca3af);"><x-icon name="bookmark" size="44" aria-label="Kayıtlı içerik yok" /></div>
    <div style="font-size:1rem;font-weight:600;margin-bottom:8px;">Henüz kaydettiğiniz içerik yok</div>
    <div style="font-size:.88rem;color:var(--u-muted,#888);margin-bottom:20px;display:inline-flex;align-items:center;gap:6px;">İçerik okurken <x-icon name="bookmark" size="14" aria-label="Kaydet" /> Kaydet butonuna basın.</div>
    @pageVisible('discover')
    <a href="{{ route('student.discover') }}" class="btn" style="display:inline-flex;align-items:center;gap:6px;"><x-icon name="compass" size="14" aria-label="Keşfet" /> İçeriklere Göz At</a>
    @endpageVisible
</div>
@else
<div class="sv-grid">
    @foreach($items as $item)
    <a href="{{ route('student.content-detail', $item->slug) }}" class="sv-card">
