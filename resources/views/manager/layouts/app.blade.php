...
<head>
    ...
    <style>
        ...
        /* Sayfa geçişleri sırasında beyaz görünmesin — html, body, main area hepsi slate */
...
<div class="app">
    ...
    <aside class="sidebar" id="premium-sidebar">
        ...
        @endphp
        @php
            $mgrLogoBg = $brandLogoBg ?? 'light';
            $mgrLogoBgStyle = match($mgrLogoBg) {
        ...
        <div style="padding:14px 14px 0;">
            ...
            <div style="display:flex;align-items:center;gap:12px;padding:10px 12px;background:rgba(255,255,255,.05);border-radius:10px;margin-bottom:12px;" title="{{ $brandName }}">
                @if(!empty($brandLogoUrl))
                    <div style="height:44px;display:flex;align-items:center;justify-content:center;flex-shrink:0;{{ $mgrLogoBgStyle }}border-radius:8px;padding:4px 6px;">
                    ...
                    <span style="display:none;width:44px;height:44px;align-items:center;justify-content:center;background:#fff;color:#1e40af;font-weight:800;border-radius:10px;font-size:18px;flex-shrink:0;">{{ $brandInitial }}</span>
                @else
                    <span style="display:flex;width:44px;height:44px;align-items:center;justify-content:center;background:#fff;color:#1e40af;font-weight:800;border-radius:10px;font-size:18px;flex-shrink:0;">{{ $brandInitial }}</span>
                @endif
                <div style="flex:1;min-width:0;">
                    ...
                    <div style="font-size:11px;color:var(--muted);font-weight:600;">Yönetici Paneli</div>
                </div>
            </div>

            {{-- User block --}}
            <div style="display:flex;align-items:center;gap:10px;">
                <div class="avatar" style="width:42px;height:42px;font-size:15px;flex-shrink:0;">{{ $mgrInitials }}</div>
                <div style="flex:1;min-width:0;">
        ...
        <nav class="sidebar-nav">
            ...
            <div class="nav-section">
                ...
                <a href="{{ route('program-search') }}"
                   class="nav-link {{ request()->is('program-search*') ? 'active' : '' }}">
                    <span class="nav-icon">🔍</span> Program Arama
                </a>
                <a href="{{ route('manager.universities.index') }}"
