...
<style>
/* ── dc-* Discover Hub ── */
.dc-hero{background:linear-gradient(to right,var(--theme-hero-from-guest) 0%,var(--theme-hero-to-guest) 100%);border-radius:14px;padding:36px 32px;color:#fff;margin-bottom:24px;}
...
.dc-search button{padding:10px 20px;background:#fff;color:var(--u-brand,#1f6fd9);border:none;border-radius:8px;font-weight:600;cursor:pointer;}
...
.dc-pill.active{background:var(--theme-hero-from-guest,#1e3a5f);color:#fff;border-color:var(--theme-hero-from-guest,#1e3a5f);font-weight:700;}
.dc-pill:hover:not(.active){background:color-mix(in srgb,var(--theme-hero-from-guest,#1e3a5f) 8%,#fff);border-color:var(--theme-hero-from-guest,#1e3a5f);color:var(--theme-hero-from-guest,#1e3a5f);}
...
.dc-pop-item:hover .dc-pop-title{color:var(--u-brand,#2563eb);}
...
/* Category badge colors — portal-aligned */
...
<div style="font-size:.83rem;margin-bottom:12px;display:flex;align-items:center;gap:6px;flex-wrap:wrap;">
    <a href="{{ route('guest.dashboard') }}" style="color:var(--theme-hero-from-guest,#1e3a5f);text-decoration:none;font-weight:500;">Ana Sayfa</a>
    <span style="color:#9ca3af;">›</span>
    <a href="{{ route('guest.discover') }}" style="color:{{ ($cat||$type||$search)?'var(--theme-hero-from-guest,#1e3a5f)':'#111827' }};text-decoration:none;font-weight:{{ ($cat||$type||$search)?'500':'700' }};">🧭 Keşfet</a>
    @if($cat)
        <span style="color:#9ca3af;">›</span>
        <a href="{{ route('guest.discover', ['cat'=>$cat]) }}" style="color:{{ $type?'var(--theme-hero-from-guest,#1e3a5f)':'#111827' }};text-decoration:none;font-weight:{{ $type?'500':'700' }};">{{ $catLabels[$cat] ?? $cat }}</a>
    @endif
    @if($type)
        <span style="color:#9ca3af;">›</span>
        <span style="color:#111827;font-weight:700;">{{ $typeLabels[$type] ?? $type }}</span>
    @endif
    @if($search && !$cat && !$type)
        <span style="color:#9ca3af;">›</span>
        <span style="color:#111827;font-weight:700;">"{{ $search }}"</span>
    @endif
</div>
...
@endphp
<div class="dc-hero" @if($cat) style="background:linear-gradient(to right,{{ $catGradientMap[$cat] ?? 'var(--theme-hero-from-guest) 0%,var(--theme-hero-to-guest) 100%' }})" @endif>
    <h1>{{ $heroTitle }}</h1>
    <p>{{ $heroSubtitle }}</p>
...
<div style="background:var(--u-card,#fff);border:1px solid var(--u-line,#e2e8f0);border-radius:12px;padding:12px 14px;margin-bottom:20px;display:flex;flex-direction:column;gap:10px;">
    ...
    <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
        ...
        <a href="{{ route('guest.discover') . ($type ? '?type='.$type : '') }}"
           class="dc-pill {{ !$cat ? 'active' : '' }}">Tümü</a>
        @foreach($catLabels as $key => $label)
            <a href="{{ route('guest.discover') . '?cat=' . $key . ($type ? '&type='.$type : '') . ($search ? '&q='.urlencode($search) : '') }}"
    ...
    <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
        ...
        <a href="{{ route('guest.discover') . ($cat ? '?cat='.$cat : '') . ($search ? ($cat?'&':'?').'q='.urlencode($search) : '') }}"
        ...
        @foreach(['blog'=>'📝 Blog','video_feature'=>'▶ Video','podcast'=>'🎙 Podcast','presentation'=>'📊 Sunum','experience'=>'💬 Deneyim','career_guide'=>'🗺 Rehber','tip'=>'💡 İpucu'] as $key => $label)
            <a href="{{ route('guest.discover') . '?type=' . $key . ($cat ? '&cat='.$cat : '') . ($search ? '&q='.urlencode($search) : '') }}"
               class="dc-pill {{ $type === $key ? 'active' : '' }}">{{ $label }}</a>
        @endforeach
        @if($cat || $type || $search)
            <a href="{{ route('guest.discover') }}" class="dc-pill" style="margin-left:auto;border-color:#ef4444;color:#ef4444;">✕ Temizle</a>
        @endif
    </div>
</div>
...
<div class="dc-featured-grid" style="margin-bottom:28px;">
    @foreach($featured as $f)
    <a href="{{ route('guest.content-detail', $f->slug) }}" class="dc-feat-card">
...
<div class="dc-layout">
    <div>
        ...
        <div class="dc-empty">
            ...
            <div style="display:flex;gap:10px;justify-content:center;flex-wrap:wrap;">
                @if($type)
                <a href="{{ route('guest.discover', $cat ? ['cat'=>$cat] : []) }}" class="btn alt" style="font-size:.85rem;">
                    {{ $cat ? ($catLabels[$cat]??$cat).' — Tümü' : 'Filtreyi Kaldır' }}
                </a>
                @endif
                @if($cat)
                <a href="{{ route('guest.discover', $type ? ['type'=>$type] : []) }}" class="btn alt" style="font-size:.85rem;">
                    {{ $type ? ($typeLabels[$type]??$type).' — Tüm Kategoriler' : 'Kategoriyi Kaldır' }}
                </a>
                @endif
                <a href="{{ route('guest.discover') }}" class="btn" style="font-size:.85rem;">Tüm İçeriklere Dön</a>
            </div>
        </div>
        @else
        <div class="dc-grid" id="dc-grid">
            @foreach($items as $item)
            <a href="{{ route('guest.content-detail', $item->slug) }}" class="dc-card" data-search="{{ strtolower($item->title_tr.' '.$item->summary_tr) }}">
    ...
    <div class="dc-sidebar">
        ...
        <div class="dc-widget">
            ...
            <a href="{{ route('guest.content-detail', $p->slug) }}" class="dc-pop-item">
        ...
        <div class="dc-widget">
            ...
            @foreach($catLabels as $key => $label)
            <a href="{{ route('guest.discover', ['cat' => $key]) }}" class="dc-pop-item">
                <div>
...
(function(){
    ...
    var timer;
    // Kullanıcı yazmayı bırakınca 500ms sonra formu server-side submit et.
    // Sayfalama ile çelişen client-side kart gizleme kaldırıldı.
