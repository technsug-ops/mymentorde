...
(function(){
    document.querySelectorAll('input[type="file"][data-fname-target]').forEach(function(inp){ inp.addEventListener('change', function(){ if(!this.files || !this.files[0]) return; var tgt=document.getElementById(this.dataset.fnameTarget); if(tgt){tgt.classList.add('show');var n=tgt.querySelector('.sf-name');if(n)n.textContent=this.files[0].name;} var f=this.closest('form'); if(f){var btn=f.querySelector('button[type="submit"]'); if(btn){btn.disabled=true;btn.textContent='Yükleniyor…';} f.submit();} }); });
    ...
    // Sil butonu — confirm + form submit (CSP-safe, inline onclick yok)
    document.querySelectorAll('[data-delete-doc]').forEach(function(btn){
        btn.addEventListener('click', function(){
            if (!confirm('Bu belgeyi silmek istediğinize emin misiniz? Bu işlem geri alınamaz.')) return;
            var f = this.closest('[data-delete-form]');
            if (f) f.submit();
        });
    });
