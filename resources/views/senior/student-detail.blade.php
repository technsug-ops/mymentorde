            ...
            @module('doc_request')
                @can('doc_request.use')
                    <button type="button" id="docReqOpenBtn"
                            style="padding:6px 12px;border:none;border-radius:6px;font-size:12px;font-weight:700;color:#fff;background:linear-gradient(135deg,#1e40af,#3b5fcc);cursor:pointer;">
                        📲 Belge Talep Et
                    </button>
                @endcan
            @endmodule
...
{{-- Aktivite & Görüşme Günlüğü (ClickUp tarzı: rapor + görsel + timeline) --}}
<div class="s360-pane" id="pane-notlar">
<article class="panel">
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px;">
        <h3 style="margin:0;">Aktivite & Görüşme Günlüğü</h3>
        <a class="btn" href="/senior/notes?student={{ $studentId }}">Tüm Notlar</a>
    </div>
    ...
    {{-- Yeni aktivite ekle --}}
    <form method="POST" action="{{ route('senior.student.activity.store', $studentId) }}" enctype="multipart/form-data"
    ...
    <div style="border-left:3px solid #7e58bf;padding:8px 0 12px 12px;margin-bottom:4px;">
        <div style="display:flex;gap:6px;align-items:center;flex-wrap:wrap;">
            ...
            @if($note->is_pinned)<span class="badge ok">📌</span>@endif
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:8px;">
            @foreach($note->attachments as $i => $att)
                @if(!empty($att['path']))
                <a href="{{ route('senior.activity.attachment', [$note->id, $i]) }}" target="_blank" rel="noopener">
                    <img src="{{ route('senior.activity.attachment', [$note->id, $i]) }}" alt="{{ $att['name'] ?? 'görsel' }}"
...
@module('doc_request')
@can('doc_request.use')
{{-- ── Belge Talep Linki Modal (öğrenci versiyonu, senior) ─────────────── --}}
    ...
    <div style="background:#fff;border-radius:14px;max-width:520px;width:100%;max-height:92vh;overflow:auto;box-shadow:0 20px 60px rgba(0,0,0,.25);">
        ...
        <div style="padding:18px 22px;">
            <p style="font-size:13px;color:#475569;line-height:1.5;margin:0 0 14px;">
                <strong>{{ $guestName ?: $studentId }}</strong> için tek-kullanımlık belge yükleme linki oluştur.
            </p>
...
(function(){
    ...
    var CSRF = '{{ csrf_token() }}';
    var INDEX_URL = "{{ route('senior.student.document-tokens.index', $studentId) }}";
    var STORE_URL = "{{ route('senior.student.document-tokens.store', $studentId) }}";
