...
<div id="paketler" class="wrap sec">
    <div class="sec-head">
        <span class="pill-lbl">Paketler</span>
        <h2 class="h2">Size uygun destek seviyesi</h2>
        @if($packageNote !== '')<p>{{ $packageNote }}</p>@endif
    </div>
    <div class="grid" style="--min:290px;--cols:{{ $cols(count($packages)) }};">

        @foreach($packages as $p)
            <div class="glass pkg{{ !empty($p['featured']) ? ' pkg-hi' : '' }}">
                ...
                @endif
                <a href="{{ $applyUrl }}" class="pkg-btn" data-track="cta_clicked" data-ph-cta-name="package_apply" data-ph-location="partner_bulut_packages">Bu paketi görüşelim</a>
            </div>
        @endforeach
    </div>
</div>
