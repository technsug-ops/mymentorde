{{-- Klasör düzenleme modalı: ad + açıklama + yetkiler --}}
<dialog id="dam-folder-edit-modal" style="border:none;border-radius:14px;padding:0;max-width:480px;width:90vw;box-shadow:0 20px 60px rgba(0,0,0,.25);">
    <form id="dam-folder-edit-form" method="POST" action="" style="padding:24px;">
        @csrf
        @method('PUT')
        <input type="hidden" name="_has_roles_form" value="1">

        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:18px;">
            <h2 style="margin:0;font-size:18px;font-weight:700;">⚙ Klasör Ayarları</h2>
        ...
        <div style="margin-bottom:14px;">
            ...
            <input type="text" id="dam-fedit-name" name="name" required maxlength="150" autofocus
                   style="width:100%;padding:10px;border:1px solid #cbd5e1;border-radius:8px;">
        </div>

        <div style="margin-bottom:14px;">
            <label style="display:block;font-size:12px;font-weight:600;margin-bottom:6px;">Açıklama (opsiyonel)</label>
            <textarea id="dam-fedit-description" name="description" rows="3" maxlength="1000"
        ...
        <div style="margin-bottom:18px;">
            ...
            <div style="padding:10px 12px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;">
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;font-size:12px;">
                    @foreach($editRoleOptions as $code => $label)
                        <label style="display:flex;align-items:center;gap:6px;cursor:pointer;">
                            <input type="checkbox" class="dam-fedit-role" name="allowed_roles[]" value="{{ $code }}">
                            <span>{{ $label }}</span>
                        </label>
                    @endforeach
                </div>
